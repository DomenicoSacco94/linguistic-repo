...
export const SentenceButton: React.FC<{
    ...
    return <>
        <RightOutlined style={{color: showTranslation ? 'red' : 'black'}} className="paragraphButton"
                       onClick={onClick}/>
        {displayedSentence.map((word: string, index) => <WordButton style={{color: showTranslation ? 'red' : 'black'}}
                                                                    disabled={showTranslation} key={index}
                                                                    word={word}/>)}
    </>
}